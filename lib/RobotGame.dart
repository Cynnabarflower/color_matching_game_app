...
import 'package:color_matching_game/NewRobot.dart';
...
class RobotsGame extends StatefulWidget {
  ...
  @override
  State<StatefulWidget> createState() => _RobotsGameState();

  RobotsGame(this.colors, {this.levels = 3});
}
...
class _RobotsGameState extends State<RobotsGame> {
  ...
  void initState() {
    ...
    robot = NewRobot(partColors: List.generate(11, (index) =>PartColor(Colors.purpleAccent)));
  ...
  Widget createMainWidget(num w, num h) {
    return Stack(
      ...
      children: [
        ...
        ..w = w
        ..h = h
//        NewRobot(w: w, h: h, partColors: List.generate(10, (index) => colors[0])),
      ],
    );
  }
  ...
  void showNext() {
    if (levelStep == 0) {
      generateRobots(answer: ANSWER.DIFF);
  ...
  void generateRobots({ANSWER answer}) {
